"""
Training using classifier loss
"""
import itertools
import os
import torch
from torch.autograd import Variable
from expr_suites.with_clf.train_logic import do_forward_pass
...
def training(params, logger, train_func=do_forward_pass, save_model_to="",model_def=sinno_model):
    ...
    for i in range(params.iterations):
        ...
        acc_code_loss, acc_discr_loss, acc_discr_acc, acc_clf_acc = 0,0,0,0
        ...
        for step, ((images_src, labels_src), (images_tgt, labels_tgt)) in zipped_loader:
            ...
            total_loss,code_loss,discr_loss,discr_acc,clf_acc = train_out["total_loss"],train_out["code_loss"],train_out["discriminator_loss"],train_out["discriminator_acc"],train_out["clf_acc"]
        ...
        clf_acc_records.append(acc_clf_acc/(step+1))
        logger.info("epoch {} | code loss: {}, discr loss: {}, discr acc: {}, clf acc: {}".format(i, acc_code_loss / (step+1),acc_discr_loss/(step+1), acc_discr_acc/(step+1),acc_clf_acc/(step+1)))
    ...
    return {
        ...
        "clf_acc_records": clf_acc_records
    }
