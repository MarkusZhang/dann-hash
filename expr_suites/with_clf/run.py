...
import params
import expr_suites.with_clf.model_2 as test_model
from utils import LoggerGenerator, save_param_module
from expr_suites.with_clf.train import training
...
params.hash_size = 16 # TODO: this changes from experiment to expr.
...
def train(params,id):
    ...
    # perform training
    train_results = training(params=params,model_def=test_model, logger=LoggerGenerator.get_logger(
    ...
    lines = [str(l) for l in train_results["clf_acc_records"]]
    plot_trend_graph(var_names=["code acc"], var_indexes=[-1], var_types=["float"], var_colors=["r"], lines=lines,
                     title="code acc", save_to=os.path.join(save_result_path, "train-code_acc.png"), show_fig=False)
